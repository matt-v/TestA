 ...
 * @author mattvaughan
 */
class SocketThread implements Runnable, JMC {
   
   private Thread t;                // for the new socket thread
   private String client;           // ip address of this thread's scratch client
   ...
   SocketThread( String host ) {

      this.client = host;
      
      t = new Thread( this, "Socket Thread: "+host);  // second argument is the thread's name
      t.start();
   }
   ...
   public void run() {
      try {
         ...
         Socket socket = new Socket( client, 42001 );
   ...
   private void parseBroadcast(String broadcast) {
       ...
       * Command#Phrase/Part Number#command specific info#command specific info#...
       * example:
   ...
   private void addNotes(String[] splitLine) {
      ...
      if ( loopInstructions.length == 2 &&               // there was exactly one dash
         ...
         // gets the phrase number which the this note should stop sounding on
         Long dieOn = new Long( loopInstructions[1] );
